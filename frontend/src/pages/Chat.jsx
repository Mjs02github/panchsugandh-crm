...
function OrderPicker({ onSelect, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl w-full max-w-sm p-5 space-y-3 animate-slide-up max-h-[80vh] flex flex-col">
                ...
                <div className="overflow-y-auto flex-1 space-y-2">
                    ...
                    {!loading && results.length === 0 && q.length > 0 && (
                        <p className="text-xs text-center text-gray-400 py-4">No orders found</p>
                    )}
                    {results.map(o => (
                        <button key={o.id} onClick={() => onSelect(o)}
                            ...
                            <p className="text-xs text-gray-500 font-mono mt-0.5">{o.order_number} • ₹{parseFloat(o.total_amount).toLocaleString('en-IN')}</p>
                            <span className="text-[10px] bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full mt-1 inline-block">{o.status}</span>
                        </button>
                    ))}
                </div>
            </div>
...
function NewChannelModal({ onCreated, onClose }) {
    ...
    return (
        <>
            <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center p-4 z-50">
                <div className="bg-white rounded-2xl w-full max-w-sm p-5 space-y-4 animate-slide-up">
                    <div className="flex justify-between items-center">
                        <h3 className="font-bold text-gray-800 text-lg">New Discussion</h3>
                        <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
                    </div>
                    ...
                    <div>
                        <label className="label text-xs">Topic / Channel Name *</label>
                        <input className="input" placeholder="e.g. Delivery issue, Pricing query..."
                            value={name} autoFocus onChange={e => setName(e.target.value)}
                            onKeyDown={e => e.key === 'Enter' && handleCreate()} />
                    </div>

                    <div>
                        <label className="label text-xs">Link to Order (optional)</label>
                        {linkedOrder ? (
                            <div className="flex items-center gap-2 p-3 bg-brand-50 border border-brand-200 rounded-xl">
                                ...
                                <button onClick={() => setLinkedOrder(null)} className="text-gray-400 hover:text-red-500 text-sm font-bold">✕</button>
                            </div>
                        ) : (
                    ...
                    <button onClick={handleCreate} disabled={creating || !name.trim()}
                        className="w-full py-3 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-sm disabled:opacity-50 transition-colors">
        ...
        <div className="flex flex-col h-screen pb-16 bg-gray-50">
            {!activeChannel ? (
                ...
                <div className="flex flex-col h-full">
                    <div className="page-header bg-white border-b border-gray-100 flex items-center justify-between px-4 py-4">
                        <div>
                            <h1 className="text-lg font-bold text-gray-800">💬 Staff Chat</h1>
                            <p className="text-xs text-gray-400">Team discussions</p>
                        </div>
                        <button onClick={() => setShowNewChannel(true)}
                            className="flex items-center gap-1.5 px-4 py-2 bg-brand-600 text-white text-sm font-semibold rounded-xl shadow-sm hover:bg-brand-700 transition-colors">
                            + New
                        </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
                        {channels.length === 0 ? (
                            ...
                            <div key={ch.id} onClick={() => setActiveChannel(ch)}
                                className="flex items-center gap-3 px-4 py-3.5 bg-white hover:bg-gray-50 cursor-pointer transition-colors">
                                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 text-lg font-bold ${ch.type === 'order' ? 'bg-brand-100 text-brand-700' : 'bg-green-100 text-green-700'}`}>
                                    {ch.type === 'order' ? '📦' : '#'}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <p className="font-semibold text-gray-800 text-sm truncate">{ch.name}</p>
                                        <p className="text-[10px] text-gray-400 shrink-0 ml-2">{fmtTime(ch.last_message_at || ch.created_at)}</p>
                                    </div>
                                    {ch.order_number && (
                                        <p className="text-[10px] text-brand-600 font-mono">{ch.order_number}</p>
                                    )}
                                    <p className="text-xs text-gray-400 truncate mt-0.5">
                                        {ch.last_message
                                            ? <><span className="font-medium text-gray-500">{ch.last_sender?.split(' ')[0]}:</span> {ch.last_message}</>
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 shrink-0">
                        ...
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg ${activeChannel.type === 'order' ? 'bg-brand-100' : 'bg-green-100'}`}>
                            {activeChannel.type === 'order' ? '📦' : '#'}
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="font-bold text-gray-800 text-sm truncate">{activeChannel.name}</p>
                            ...
                            return (
                                <div key={msg.id} className={`flex gap-2 ${mine ? 'flex-row-reverse' : ''}`}>
                                    ...
                                    <div className={`max-w-[75%] ${mine ? 'items-end' : 'items-start'} flex flex-col`}>
                                        ...
                                        <p className={`text-[9px] text-gray-400 mt-0.5 px-1 ${mine ? 'text-right' : ''}`}>
                    ...
                    <div className="bg-white border-t border-gray-100 px-3 py-3 flex items-end gap-2 shrink-0">
                        ...
                        >
                            {sending ? (
                                ...
                                <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 rotate-45 -ml-0.5 -mt-0.5">
            ...
            {showNewChannel && (
                <NewChannelModal
                    onCreated={ch => { setShowNewChannel(false); loadChannels(); setActiveChannel(ch); }}
                    onClose={() => setShowNewChannel(false)}
                />
            )}

            {!activeChannel && <BottomNav />}
        </div>
    );
}
